...
app.post('/api/orders', lib.isLoggedIn, async function(req, res) {
  ...
  try {
    ...
    user.orders.push({
        ...
        totalPriceTotal   : (Number(req.session.totalPrc)+Number(req.session.shippingCost)-Number(req.session.pointDiscount)-Number(req.session.shippingDiscount)).toFixed(2),
